defmodule ReportGeneratorTest do
  ...
  describe "build_from_many/1" do
    ...
    test "when a file list is not provided, returns an error" do
      response = ReportGenerator.build_from_many("whatever")
  ...
  describe "fetch_higher_cost/2" do
    test "when the option is 'users', returns the user who spented more" do
      file_name = "report_test.csv"

      response =
        file_name |> ReportGenerator.build() |> ReportGenerator.fetch_higher_cost("users")
    ...
    test "when the option is 'foods', returns the most consumed food" do
      ...
      response =
        file_name |> ReportGenerator.build() |> ReportGenerator.fetch_higher_cost("foods")
    ...
    test "when invalid option is given, returns an error" do
      ...
      response = file_name |> ReportGenerator.build() |> ReportGenerator.fetch_higher_cost("xyz")
